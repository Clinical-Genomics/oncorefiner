...
- [#130](https://github.com/Clinical-Genomics/oncorefiner/pull/130) Changed output logic for `PROCESS_SNVS` to output the `research_filtered_vcf` file after annotation with vep. The file name for this output is now given by VEP, i.e. with prefix `${meta.id}_vep` as is specified in the config file.
