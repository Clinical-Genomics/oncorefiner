...
- Place test or dataset specific arguments as parameters (ex. in `test_base.config` or `nextflow.config`) that can be accessed by `conf/*`.
- If there are tools that should support specific extra arguments (ex. that are relevant to test runs or often tailored to dataset) consider creating dedicated parameters such as `extra_args_cadd_annotate`. The parameters can then be defined for test or pipeline runs in config files (ex.`test_base.config` or `nextflow.config`).
