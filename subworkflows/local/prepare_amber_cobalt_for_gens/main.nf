...
workflow PREPARE_AMBER_COBALT_FOR_GENS {
    ...
    emit:
    gens_baf_normal_bed = PREPARE_AMBER_FOR_GENS.out.normal_bed // channel: [val(meta), path(bed)]
    gens_baf_normal_tbi = PREPARE_AMBER_FOR_GENS.out.normal_tbi // channel: [val(meta), path(tbi)]
    gens_baf_tumor_bed  = PREPARE_AMBER_FOR_GENS.out.tumor_bed  // channel: [val(meta), path(bed)]
